...
func addKernelAddr(node, iface, cidr, proto string) {
	GinkgoHelper()
	Byf("Adding address %s proto %s on %s/%s", cidr, proto, node, iface)
	output, err := runner.RunAtNode(node, "sudo", "ip", "addr", "add", cidr, "proto", proto, "dev", iface)
...
func kernelAddrInfo(node, ip string) kernelAddr {
	GinkgoHelper()
...
func skipIfKernelLacksIFAProto(node, ip string) {
	GinkgoHelper()
	info := kernelAddrInfo(node, ip)
	if info.Protocol == "" {
		Skip(fmt.Sprintf("IFA_PROTO not reported for %s on %s; kernel may be older than 5.18", ip, node))
	}
}
...
func nmDeviceAddressesIfPresent(node, iface, family string) (string, bool) {
	...
	output, err = runner.RunAtNode(node, "sudo", "nmcli", "-g", family+".addresses", "connection", "show", conn)
	if err != nil {
		return "", false
	}
...
func nnsAddressProtocol(node, iface, ip, family string) string {
	path := fmt.Sprintf("interfaces.#(name==%q).%s.address.#(ip==%q).protocol", iface, family, ip)
