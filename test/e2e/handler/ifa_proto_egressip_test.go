...
var _ = Describe("OVN-Kubernetes EgressIP IFA_PROTO", func() {
	...
	BeforeEach(func() {
		...
		// Wait until NNS has the Other-protocol address so the following
		// routes-only apply does not race a stale current-state clone.
		Eventually(func() string {
			return nnsAddressProtocol(node, dummyName, egressIPAddr, "ipv4")
