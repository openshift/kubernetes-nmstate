...
var _ = Describe("NodeNetworkConfigurationPolicy controller predicates", func() {
	...
	Describe("allPolicies function", func() {
		It("should return policies in alphanumerical order by name", func() {
			...
			allPolicies := reconciler.allPolicies()
			...
			requests := allPolicies(context.TODO(), node)
			...
			Expect(requests).To(HaveLen(len(expectedOrder)))
			for i, expectedName := range expectedOrder {
				Expect(requests[i].NamespacedName.Name).To(Equal(expectedName))
			}
		})
